...
def edge_detection(img):
	...
	CannyBoard = cv.Canny(img, 50, 285)
...
def recalibrate_pickle():

	h = 1080
	w = 1920
	i = 0
	prefix = 'calibration_dilation'
	suffix = '.JPG'
	filename = CWD + 'data/image_collection/' + prefix + suffix
	filename1 = CWD + 'data/image_collection/' + prefix + '_houghline_' + suffix
	...
	with picamera.PiCamera() as camera:
		...
		while( i < 1 ):
			...
			with picamera.array.PiRGBArray(camera) as stream:
				...
				horizontal_lines = list( line_transform( image ) )
				rect, splits = boundaries( horizontal_lines )

				rectangle_list, v_lines, h_lines = rect_parse(rect, splits)
				chessboard_img = draw_final_chessboard( v_lines, h_lines, rect, image )
